...
import { mapWpPostsToTiles, Tile } from '@/lib/adapters/mapWpToTile';

const fetcher = (url: string) => fetch(url).then(r => r.json());
const PAGE = 18;

export function useWpTiles(q: string = '', category: string = '') {
  const { data, setSize, isLoading, error } = useSWRInfinite(
    (index, prev) => {
      ...
      return `/api/posts?limit=${PAGE}${catParam}${search}${cursor}`;
    },
    fetcher,
