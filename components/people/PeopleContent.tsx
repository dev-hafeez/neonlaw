...
import { usePeopleTiles } from "@/lib/hooks/usePeopleTiles";
...
export default function PeopleContent() {
  ...
  const cats: (string | number)[] = searchParams?.get("cat")
    ? [searchParams.get("cat")!]
    : [];

  const searchKey = useMemo(() => `${q}-${cats.join(",")}`, [q, cats]);
  ...
  const { tiles, hasMore, setSize, isLoading } = usePeopleTiles(
    q,
    cats,
    searchKey
  );
